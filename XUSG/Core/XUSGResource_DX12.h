...
namespace XUSG
{
	...
	protected:
		bool createSRVs(uint16_t arraySize, Format format, uint8_t numMips, bool multisamples,
			bool isCubeMap, uint16_t srvComponentMapping);
		bool createSRVLevels(uint16_t arraySize, uint8_t numMips, Format format, bool multisamples,
			bool isCubeMap, uint16_t srvComponentMapping);
		bool createUAVs(uint16_t arraySize, Format format, uint8_t numMips);
		bool createPackedUAVs(uint16_t arraySize, Format format , uint8_t numMips);
	...
	public:
		...
		Descriptor CreateRTV(uint16_t arraySize, uint16_t firstArraySlice = 0,
	...
	public:
		...
		Descriptor CreateDSV(uint16_t arraySize, uint16_t firstArraySlice = 0,
			Format format = Format::UNKNOWN, uint8_t mipLevel = 0, bool multisamples = false,
	...
	public:
		...
		Descriptor CreateSRV(Format format = Format::UNKNOWN, uint8_t numMips = 1, uint8_t mostDetailedMip = 0,
			uint16_t srvComponentMapping = XUSG_DEFAULT_SRV_COMPONENT_MAPPING);
		Descriptor CreateUAV(uint16_t wSize, uint16_t firstWSlice = 0, Format format = Format::UNKNOWN,
			uint8_t mipLevel = 0);
	...
	public:
		...
		Descriptor CreateSRV(uint32_t numElements, uint32_t byteStride, Format format,
			uint32_t firstElement = 0, uint16_t srvComponentMapping = XUSG_DEFAULT_SRV_COMPONENT_MAPPING);
		Descriptor CreateUAV(uint32_t numElements, uint32_t byteStride, Format format,
			uint32_t firstElement = 0, size_t counterByteOffset = 0);
