	...
	PipelineLayoutLib* pPipelineLayoutLib, PipelineLayoutFlag flags, const wchar_t* name)
{
	...
	H_RETURN(pDxDevice->D3D12SerializeVersionedRootSignature(&rootSignatureDesc, signature.put(), error.put(),
		g_numUAVs), cerr, reinterpret_cast<char*>(error->GetBufferPointer()), nullptr);
