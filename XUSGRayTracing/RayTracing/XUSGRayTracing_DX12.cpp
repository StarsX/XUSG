...
D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS XUSG::RayTracing::GetDXRBuildFlags(BuildFlag buildFlags)
{
	...
	flags |= APPEND_BUILD_FLAG(buildFlags, ALLOW_OMM_UPDATE);
	flags |= APPEND_BUILD_FLAG(buildFlags, ALLOW_DISABLE_OMMS);
...
D3D12_RAYTRACING_INSTANCE_FLAGS XUSG::RayTracing::GetDXRInstanceFlags(InstanceFlag instanceFlags)
{
	...
	flags |= APPEND_INSTANCE_FLAG(instanceFlags, FORCE_OMM_2_STATE);
	flags |= APPEND_INSTANCE_FLAG(instanceFlags, DISABLE_OMMS);
